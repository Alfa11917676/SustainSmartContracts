...
contract SustainSigner is EIP712Upgradeable{
    ...
    string private  SIGNING_DOMAIN; //= "8080-Marketplace";
    ...
    function _hash(Sustain memory sustain) internal view returns (bytes32) {
        return _hashTypedDataV4(keccak256(abi.encode(
                keccak256("Sustain(address userAddress,uint256 tokenType,uint256 nonce,uint256 randomNumber)"),
                    sustain.userAddress,
                    sustain.tokenType,
                    sustain.nonce,
                    sustain.randomNumber
            )));
    }
